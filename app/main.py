...
def render_edit_subscription_modal():
    """渲染编辑订阅弹窗"""
    # 检查是否有订阅需要编辑
    ...
    for sub in subscriptions:
        if st.session_state.get(f"edit_subscription_{sub['id']}", False):
            with st.form(f"edit_subscription_form_{sub['id']}"):
                st.subheader(f"✏️ 编辑订阅: {sub['service_name']}")

                col1, col2 = st.columns(2)
                with col1:
                    service_name = st.text_input(
                        "服务名称",
                        value=sub.get("service_name", ""),
                        placeholder="例如: Netflix"
                    )
                    price = st.number_input(
                        "价格",
                        min_value=0.0,
                        value=float(sub.get("price", 0)),
                        step=0.01
                    )
                ...
                with col2:
                    billing_cycle = st.selectbox(
                        "付费周期",
                        ["monthly", "yearly", "weekly", "daily"],
                        index=["monthly", "yearly", "weekly", "daily"].index(sub.get("billing_cycle", "monthly")),
                        format_func=lambda x: {
                            "monthly": "月付", "yearly": "年付",
                            "weekly": "周付", "daily": "日付"
                        }[x]
                    ...
                    current_category = sub.get("category", "other")
                ...
                with col1:
                    from datetime import datetime
                    # 获取现有的开始日期或使用创建日期
                    existing_start_date = sub.get("start_date") or sub.get("created_at")
                ...
                with col2:
                    status = st.selectbox(
                        ...
                        index=["active", "paused", "cancelled"].index(sub.get("status", "active")),
                        format_func=lambda x: {
                            "active": "活跃",
                            "paused": "暂停",
                            "cancelled": "已取消"
                ...
                notes = st.text_area("备注", value=sub.get("notes", "") or "", placeholder="可选备注信息")
                ...
                with col1:
                    if st.form_submit_button("💾 保存", use_container_width=True):
                        if service_name and price >= 0:
                            ...
                            success = data_manager.update_subscription(sub['id'], updates)

                            if success:
                                st.success(f"✅ 成功更新订阅: {service_name}")
                                st.session_state[f"edit_subscription_{sub['id']}"] = False
                                st.rerun()
                            else:
                                st.error("❌ 更新订阅失败")
                        else:
                ...
                with col3:
                    if st.form_submit_button("🗑️ 删除", use_container_width=True, type="secondary"):
                        if data_manager.delete_subscription(sub['id']):
                            st.success(f"✅ 已删除订阅: {sub['service_name']}")
                            st.session_state[f"edit_subscription_{sub['id']}"] = False
                            st.rerun()
                        else:
                            st.error("❌ 删除失败")
...
def main():
    ...
    try:
        ...
        # render_edit_subscription_modal()  # 已改为内联编辑，不再需要模态窗口
